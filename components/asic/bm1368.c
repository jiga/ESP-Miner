...
void BM1368_send_hash_frequency(int id, float target_freq, float max_diff) {
    ...
    if (id != -1) {
        freqbuf[0] = id * 2;
        _send_BM1368(TYPE_CMD | GROUP_SINGLE | CMD_WRITE, freqbuf, 6, BM1368_SERIALTX_DEBUG);
    } else {
...
static void do_frequency_ramp_up(float target_frequency) {
    ...
    BM1368_send_hash_frequency(-1, current, 0.001);
